...
impl SecretVault {
    ...
    pub fn add_secret(&mut self, encryption_key: &[u8; 32], key: String, secret: String) {
        ...
        let str_encrypted_secret: String = encrypted_secret.iter().map(|&c| c as char).collect();

        println!("{:?}",nonce);
        println!("{}",str_encrypted_secret);
        let mut combined = nonce.to_vec();
        combined.extend(encrypted_secret);
        self.secrets.insert(key, combined);
        //self.secrets.insert(key, encrypted_secret);
    }
